...
csr_interactions = csr_matrix((df_interactions.rating.astype('float'), (df_interactions.user_id, df_interactions.book_id)), shape=(len(user_to_idx), len(book_to_idx)))
...
def flatten_column(df, id_vars, column_to_flatten):
    return pd.concat([df[id_vars], df[column_to_flatten].apply(pd.Series)], axis=1).melt(
        id_vars=id_vars, value_name=column_to_flatten).dropna().drop(
        'variable', axis=1)
...
user_book_ratings_dict = dict(zip(zip(df_train.user_id, df_train.book_id), df_train.rating))
...
book_to_series = {book_id: series for series, book_list in series_dict.items() for book_id in book_list}
...
def same_series(x, book_to_series, series_to_books):
    ...
    ratings = [user_book_ratings_dict[(x.user_id, book_id)] for book_id in same_series_books
...
df_books.shelves_set = df_books.popular_shelves.map(lambda x: set([pair['name'] for pair in x]))
shelf_rating_dicts = {}
...
def get_shelf_lf(shelf):
    name = f"shelf_{shelf}"
    return LabelingFunction(name, f=f, resources={"ratings_dict": shelf_rating_dicts[shelf]})
...
user_author_ratings = df_train.merge(df_books[['book_id', 'first_author']], on='book_id')
...
def get_feedforward_model():
    ...
    r_emb = tf.keras.layers.Embedding(6, bi_emb_dim, name='r_emb')(ratings)  # 6 because ratings are 1 to 5.
    ...
    bir_emb_reduced = tf.math.divide(tf.keras.backend.sum(bir_emb, axis=1), tf.expand_dims(num_b, 1), name='bir_emb_reduced')
    #### Same as other model.
    input_layer = tf.keras.layers.concatenate([bir_emb_reduced, b_emb], 1, name='input_layer')
    ...
    feedforward_model = tf.keras.Model(inputs=[num_b, bids, ratings, bid], outputs=[output_layer])
    feedforward_model.compile('Adagrad', 'sparse_categorical_crossentropy', metrics=['accuracy'])
...
def parse_row(row, max_len=40):
    ...
    return {'num_b': len(row), 'bids': bids, 'ratings': ratings, 'bid': bid, 'label': rating - 1}
...
def get_data_tensors(data_array):
    ...
    return (tensor_dict["num_b"], tensor_dict["bids"], tensor_dict["ratings"], tensor_dict["bid"], tensor_dict["label"])
...
feedforward_model.fit(
    train_data_tensors[:-1], train_data_tensors[-1], steps_per_epoch=10000,
    validation_data=(val_data_tensors[:-1], val_data_tensors[-1]), validation_steps=1000,
    callbacks=[early_stopping], epochs=1, verbose=1)
...
def get_feedback_model():
    ...
    feedback_model.compile('Adagrad', 'sparse_categorical_crossentropy', metrics=['accuracy'])
...
feedback_model.fit([df_train.user_id.values, df_train.book_id.values], df_train.rating - 1,
                   validation_data=((df_val.user_id.values, df_val.book_id.values), df_val.rating - 1),
                   callbacks=[early_stopping], epochs=20, verbose=1)
base_score = feedback_model.evaluate([df_test.user_id.values, df_test.book_id.values], df_test.rating - 1)

# %%
# Learn on combined data
#feedback_model_combined = get_feedback_model()
feedback_model_combined.fit([combined_df_train.user_id.values, combined_df_train.book_id.values], combined_df_train.rating - 1,
                   validation_data=((df_val.user_id.values, df_val.book_id.values), df_val.rating - 1),
                   callbacks=[early_stopping], epochs=30, verbose=1)
lf_score = feedback_model_combined.evaluate([df_test.user_id.values, df_test.book_id.values], df_test.rating - 1)
...
csr_interactions = csr_matrix((df_interactions.rating.map({1: 0., 2: 0., 3: 0., 4:1., 5:1.}),
...
# %%
for i, (u, b, r) in enumerate(zip(df_interactions.user_id, df_interactions.book_id, df_interactions.rating)):
